  ...
  def observe(self, observation, gameState):
    ...
    if noisyDistance != None:
        for legal in self.legalPositions:
            distance = util.manhattanDistance(legal, pacmanPosition)
            possible[legal] = emissionModel[distance] * self.beliefs[legal]
    else: possible[self.getJailPosition()] = 1
...
class ParticleFilter(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    # if we are in jail, make sure we set our self as such
    if noisyDistance == None:
        position = self.getJailPosition()
        possible[position] = self.numParticles
    else:
        for legal in self.legalPositions:
            distance = util.manhattanDistance(legal, pacmanPosition)
            possible[legal] += emissionModel[distance] * self.beliefs[legal]
...
class JointParticleFilter:
    ...
    def elapseTime(self, gameState):
        ...
        for positions in self.particles:
            particle = list(positions)
            newstate = setGhostPositions(gameState, particle)
            for ghost in range(self.numGhosts):
                agent  = self.ghostAgents[ghost]
                distri = getPositionDistributionForGhost(newstate, ghost, agent)
                particle[ghost] = util.sample(distri)
    ...
    def observeState(self, gameState):
        ...
        for particle in self.particles:
            ...
            for ghost, position in enumerate(particle):
                ...
                else: nparticle.append(self.getJailPosition(ghost))
        ...
        #------------------------------------------------------------ 
        # Try by taking running particle filter on each position in
        # each particle individually
        #------------------------------------------------------------ 
        #beliefs = self.getBeliefDistribution()

        # belief prior for each ghost individually
        #beliefs = [util.Counter() for _ in range(self.numGhosts)]
        #for particle in self.particles:
        #    for ghost, position in enumerate(particle):
        #        beliefs[ghost][position] += 1
        #[b.normalize() for b in beliefs]

        # belief prior for all positions together
        ...
        # update the weights
        #possibles = [util.Counter() for _ in range(self.numGhosts)]
        ##for particle in self.particles:
        ##    for ghost, position in enumerate(particle):
        ...
        #            distance = util.manhattanDistance(position, pacmanPosition)
        #            evidence = emissionModels[ghost][distance]
        #            #evidence = evidence * beliefs[position]
        #            evidence = evidence * beliefs[ghost][position]
        #            possibles[ghost][position] += evidence 

        ## add ghosts in jail
        #for ghost in range(self.numGhosts):
        #    if noisyDistances[ghost] == None:
        #        jail = self.getJailPosition(ghost)
        #        possibles[ghost][jail] = self.numParticles
        #[p.normalize() for p in possibles]

        ## if we don't know where anyone is, start over
        #if any(p.totalCount() == 0.0 for p in possibles):
        #    self.initializeParticles()
        #    gparticles = []
        #    for positions in self.particles:
        #        particle = list(positions)
        ...
        #        gparticles.append(tuple(particle))
        #    self.particles = gparticles
        #    return 
        ...
        #    particle = []
        #    for ghost in range(self.numGhosts):
        #        particle.append(util.sample(possibles[ghost]))
        #    self.particles.append(tuple(particle))
        ...
        #            evidence = emissionModels[ghost][distance]
        #            evidence = evidence * beliefs[position]
        #            possibles[position] += evidence 

        ## add ghosts in jail
        #for ghost in range(self.numGhosts):
        #    if noisyDistances[ghost] == None:
        #        jail = self.getJailPosition(ghost)
        #        possibles[jail] = self.numParticles
        #possibles.normalize()

        ## if we don't know where anyone is, start over
        #if possibles.totalCount() == 0.0:
        #    print "~",
        #    self.initializeParticles()
        #    return 
        ...
        #    particle = []
        #    for ghost in range(self.numGhosts):
        #        particle.append(util.sample(possibles))
        #    self.particles.append(tuple(particle))
