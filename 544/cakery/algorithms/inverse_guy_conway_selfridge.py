...
class InverseGuyConwaySelfridge(FairDivider):
    '''
    '''
    ...
    def divide(self):
        ...
        value1, piece1 = values.pop(0)                                  # the smallest piece and value
        ...
        for user in [picker, marker, cutter]:                           # the users choose in this order
            ...
            else: slices[user].append(choose_worst_piece(user, pieces)) # otherwise, pick the worst user piece
        ...
        if trimming:                                                    # if there was a trimming
            ...
            for user in [chooser, trimmer, cutter]:                     # the users choose in this order
                slices[user].append(choose_worst_piece(user, pieces))   # the worst piece that each user sees
        return slices
