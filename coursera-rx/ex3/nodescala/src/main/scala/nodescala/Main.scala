...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    val myServerSubscription = myServer.start("/test"){ r =>
      r.keys.toIterator
    }
    ...
    val userInterrupted: Future[String] = Future
      .userInput("something")
      .continueWith { x => "You entered... " + x.now }
    ...
    val timeOut: Future[String] = Future
      .delay(20 seconds)
      .continueWith { _ => "Server timeout!" }

    // TO IMPLEMENT
    // 4. create a future that completes when either 10 seconds elapse
    //    or the user enters some text and presses ENTER
    val terminationRequested: Future[String] = Future
      .any(List(userInterrupted, timeOut))
