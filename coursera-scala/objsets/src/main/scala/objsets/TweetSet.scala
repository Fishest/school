...
abstract class TweetSet {
  ...
  def union(that: TweetSet): TweetSet =
    if (isEmpty) that else tail union (that incl head)

  // Hint: the method "remove" on TweetSet will be very useful.
  def ascendingByRetweet: Trending = {
    def iterate(accu: Trending, tweets: TweetSet) : Trending =
      if (tweets.isEmpty) accu
      else iterate(accu + tweets.findMin, tweets.remove(tweets.findMin))
    iterate(new EmptyTrending, this)
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet = {
    val naccu = if (p(elem)) accu.incl(elem) else accu
    val laccu = left.filter0(p, naccu) 
    right.filter0(p, laccu)
  }
...
object GoogleVsApple {
  ...
  val googleTweets: TweetSet = TweetReader.allTweets
    .filter(x => google.exists(x.text.contains))

  val appleTweets: TweetSet = TweetReader.allTweets
    .filter(x => apple.exists(x.text.contains))
